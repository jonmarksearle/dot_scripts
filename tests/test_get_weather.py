...
class ForecastWindowStub(get_weather.ForecastWindow):
    def __init__(self, dates: Iterable[date]):
        super().__init__(dates=tuple(dates))
...
def mk_daily(
    d: date,
    src: str = "Test",
    min_t: Optional[float] = 20.0,
    max_t: Optional[float] = 30.0,
    min_w: Optional[float] = 10.0,
    max_w: Optional[float] = 20.0,
    w_dir: Optional[str] = "N",
    prog: Optional[str] = "CLEAR",
    rain_p: Optional[float] = 0.0,
) -> get_weather.DailyData:  # type: ignore
    # Convert string prognosis to WeatherCode if it matches
    prog_enum = None
    if prog:
        try:
            prog_enum = get_weather.WeatherCode(prog)
        except ValueError:
            pass  # Keep None or handle error
...
def test__taxonomy__map_wmo__unknown_code__fail() -> None:
    ...
    with pytest.raises(ValueError, match=r"Unknown WMO code"):
        get_weather.map_wmo_code(9999)


@pytest.mark.parametrize(
    "code,expected",
    [
        (0, get_weather.WeatherCode.CLEAR),
        (3, get_weather.WeatherCode.CLOUDY),
        (61, get_weather.WeatherCode.RAIN),
        (95, get_weather.WeatherCode.STORM),
    ],
)
def test__taxonomy__map_wmo__success(code: int, expected: str) -> None:
    """Ensure WMO codes map to standard taxonomy."""
    assert get_weather.map_wmo_code(code) == expected


@pytest.mark.parametrize(
    "text,expected",
    [
        ("Sunny", get_weather.WeatherCode.CLEAR),
        ("Partly cloudy", get_weather.WeatherCode.CLOUDY),
        ("Patchy rain nearby", get_weather.WeatherCode.RAIN),
    ],
)
def test__taxonomy__map_wttr__success(text: str, expected: str) -> None:
    """Ensure wttr.in strings map to standard taxonomy."""
    assert get_weather.map_wttr_text(text) == expected


@pytest.mark.parametrize(
    "text,expected",
    [
        ("Fine", get_weather.WeatherCode.CLEAR),
        ("Cloudy", get_weather.WeatherCode.CLOUDY),
        ("Showers", get_weather.WeatherCode.RAIN),
        ("Possible storm", get_weather.WeatherCode.STORM),
    ],
)
def test__taxonomy__map_bom__success(text: str, expected: str) -> None:
    """Ensure BOM text strings map to standard taxonomy."""
    assert get_weather.map_bom_text(text) == expected


@pytest.mark.parametrize(
    "candidates,expected",
    [
        (["CLEAR", "RAIN"], get_weather.WeatherCode.RAIN),
        (["SNOW", "RAIN"], get_weather.WeatherCode.SNOW),
        (["STORM", "SNOW"], get_weather.WeatherCode.STORM),
        (["CLOUDY", "CLEAR"], get_weather.WeatherCode.CLOUDY),
        (["RAIN", "CLOUDY"], get_weather.WeatherCode.RAIN),
    ],
)
def test__taxonomy__severity_ranking__success(
    candidates: list[str], expected: str
) -> None:
    """Ensure pick_worst correctly resolves pairwise severity (STORM > SNOW > RAIN > CLOUDY > CLEAR)."""
    enum_candidates = [get_weather.WeatherCode(c) for c in candidates]
    assert get_weather.pick_worst(enum_candidates) == expected
...
def test__consensus__empty_input__success(window_stub: ForecastWindowStub) -> None:
    assert (
        get_weather.calculate_consensus(window_stub, [], get_weather.ConsensusPolicy())
        == []
    )


def test__consensus__date_omission__all_fields_none__success(
    window_stub: ForecastWindowStub, base_date: date
) -> None:
    ...
    results = get_weather.calculate_consensus(
        window_stub, data, get_weather.ConsensusPolicy()
    )
    assert len(results) == 1
    assert results[0].date == str(base_date)


def test__consensus__sources_contribution__ignore_empty__success(
    window_stub: ForecastWindowStub, base_date: date
) -> None:
    """Ensure a source is NOT listed if it contributes no valid data."""
    data = mk_daily_list(base_date, {"src": "A"}, {"src": "B", "all_none": True})
    res = get_weather.calculate_consensus(
        window_stub, data, get_weather.ConsensusPolicy()
    )[0]
...
def test__consensus__aggregation__temp__ignore_none__success(
    window_stub: ForecastWindowStub, base_date: date
) -> None:
...
def test__consensus__aggregation__wind_range__success(
    window_stub: ForecastWindowStub, base_date: date
) -> None:
    """Ensure wind range captures absolute min and max across sources."""
    data = mk_daily_list(
        base_date, {"min_w": 10, "max_w": 20}, {"min_w": 5, "max_w": 30}
    )
    res = get_weather.calculate_consensus(
        window_stub, data, get_weather.ConsensusPolicy()
    )[0]
...
def test__consensus__aggregation__wind_range__ignore_none__success(
    window_stub: ForecastWindowStub, base_date: date
) -> None:
    """Ensure None values in wind are ignored for Range calculation."""
    data = mk_daily_list(
        base_date, {"min_w": 10, "max_w": 20}, {"min_w": None, "max_w": None}
    )
    res = get_weather.calculate_consensus(
        window_stub, data, get_weather.ConsensusPolicy()
    )[0]
...
def test__consensus__aggregation__rain_prob__max_success(
    window_stub: ForecastWindowStub, base_date: date
) -> None:
...
def test__consensus__aggregation__rain_prob__ignore_none__success(
    window_stub: ForecastWindowStub, base_date: date
) -> None:
...
def test__consensus__aggregation__direction__union_and_sort__success(
    window_stub: ForecastWindowStub, base_date: date
) -> None:
...
def test__consensus__aggregation__direction__ignore_none__success(
    window_stub: ForecastWindowStub, base_date: date
) -> None:
...
def test__consensus__aggregation__prognosis__mode_and_tiebreaker__success(
    window_stub: ForecastWindowStub, base_date: date
) -> None:
    ...
    assert res.prognosis == get_weather.WeatherCode.STORM


def test__consensus__aggregation__direction__all_none_is_none__success(
    window_stub: ForecastWindowStub, base_date: date
) -> None:
    """Ensure if ALL directions are None, result is None (not empty list)."""
    data = mk_daily_list(
        base_date, {"min_t": 20, "w_dir": None}, {"min_t": 20, "w_dir": None}
    )
    res = get_weather.calculate_consensus(
        window_stub, data, get_weather.ConsensusPolicy()
    )[0]
...
def test__consensus__outlier__count_2__no_removal__success(
    window_stub: ForecastWindowStub, base_date: date
) -> None:
...
def test__consensus__outlier__stddev_0__no_removal__success(
    window_stub: ForecastWindowStub, base_date: date
) -> None:
...
def test__consensus__outlier__removal__success(
    window_stub: ForecastWindowStub, base_date: date
) -> None:
    """Ensure outlier removed if count > 2 and deviation > 1.5 sigma."""
    # Data: [20, 20, 20, 20, 100]. Mean (with outlier): 36. StdDev: ~35.7.
    # Bound: 36 + 1.5*35.7 = 89.5. 100 > 89.5.
    data = mk_daily_list(
        base_date,
        {"max_t": 20},
        {"max_t": 20},
...
def test__consensus__outlier_ignore_nones__success(
    window_stub: ForecastWindowStub, base_date: date
) -> None:
...
def test__consensus__grouping_determinism__success(
    window_stub: ForecastWindowStub, base_date: date
) -> None:
    """Ensure unsorted mixed-date inputs produce sorted outputs."""
    data = mk_daily_list(
        base_date, {"offset": 1, "src": "A"}, {"offset": 0, "src": "B"}
    )
    res = get_weather.calculate_consensus(
        window_stub, data, get_weather.ConsensusPolicy()
    )
    assert len(res) == 2 and (res[0].date, res[1].date) == (
        str(base_date),
