...
def mk_daily(
    ...
    return get_weather.DailyData(
        ...
        direction=w_dir, prognosis=prog_enum,
        rain_prob=rain_p
    )
...
def test__consensus__empty_input__success(window_stub: ForecastWindowStub) -> None:
    assert get_weather.calculate_consensus(window_stub, [], get_weather.ConsensusPolicy()) == []
...
def test__consensus__date_omission__all_fields_none__success(window_stub: ForecastWindowStub, base_date: date) -> None:
    ...
    results = get_weather.calculate_consensus(window_stub, data, get_weather.ConsensusPolicy())
...
def test__consensus__sources_contribution__ignore_empty__success(window_stub: ForecastWindowStub, base_date: date) -> None:
    ...
    res = get_weather.calculate_consensus(window_stub, data, get_weather.ConsensusPolicy())[0]
...
def test__consensus__outlier__removal__success(window_stub: ForecastWindowStub, base_date: date) -> None:
    ...
    data = mk_daily_list(base_date, {"max_t": 20}, {"max_t": 20}, {"max_t": 20}, {"max_t": 20}, {"max_t": 100})
    res = get_weather.calculate_consensus(window_stub, data, get_weather.ConsensusPolicy())[0]
    assert res.max_temp == 20.0
...
def test__consensus__grouping_determinism__success(window_stub: ForecastWindowStub, base_date: date) -> None:
    ...
    res = get_weather.calculate_consensus(window_stub, data, get_weather.ConsensusPolicy())
    assert len(res) == 2 and (res[0].date, res[1].date) == (str(base_date), str(base_date + timedelta(days=1)))
